...
public:
    ...
    static unsigned char * util_stbi_load_from_memory(unsigned char const * buffer, int len, int * x, int * y,
                                           int * channels_in_file, int desired_channels);

    static Vec2<> calcKinematicPos(const Vec2<> & pos, const Vec2<> & vel, const Vec2<> & acc, double deltaTime);
    static Vec2<> calcKinematicVel(const Vec2<> & vel, const Vec2<> & acc, double deltaTime);
private:
