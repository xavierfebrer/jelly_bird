...
unsigned char *
Util::util_stbi_load(char const * filename, int * x, int * y, int * comp, int req_comp) {
...
unsigned char * Util::util_stbi_load_from_memory(unsigned char const * buffer, int len, int * x, int * y,
                                      int * channels_in_file, int desired_channels) {
    return stbi_load_from_memory(buffer, len, x, y, channels_in_file, desired_channels);
}

Vec2<> Util::calcKinematicPos(const Vec2<> & pos, const Vec2<> & vel, const Vec2<> & acc,
                              double deltaTime) {
    return Vec2<>(pos + (vel * deltaTime) + (acc * 0.5 * (deltaTime * deltaTime)));
}

Vec2<> Util::calcKinematicVel(const Vec2<> & vel, const Vec2<> & acc, double deltaTime) {
    return Vec2<>(vel + (acc * deltaTime));
}
